...
#include <stdlib.h> //Para declarar macros
#include <string.h> //biblioteca la cual permite comparar cadenas de caracteres
#include <time.h> //biblioteca con la que se puede declarar Srand(time(0))
...
// Prototipos
int repo_existe(); // Funcion tipo entero, verifica que existe el repositorio en el directorio
int crear_carpeta_repo(); // Funcion que crea la carpeta repositorio
void crear_archivos_control();   
void ugit_init(); // Inicia el repositorio 

FILE *abrir_staging_para_lectura(void); 
void vaciar_staging(void);

void generar_hash(char *dest, size_t len); // Se crea la id del commit
void ugit_add(const char *archivo_commit); // funcion para agrega commits al staging 
void ugit_commit(const char *mensaje); 
void cargar_commits_desde_archivo(void);  
void ugit_log(void); // Funcion que devuelve informacion del commit reciente
void ugit_checkout(const char *commit_id); // Funcion que que busca un commit por su id
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "init") == 0) {
        ...
        if (argc < 4 || strcmp(argv[2], "-m") != 0) // Sentencia que comprueba quu se hayan pasado menos de 4 parametros o que el segundo no sea -m 
            printf("Uso: ugit commit -m \"mensaje\"\n"); // si una de las condiciones se cumple se le muestra al usuario como ejecutar correctamente ugit commit 
        else
            ugit_commit(argv[3]); // Se ejecuta la funcion ugit_commit
    }
...
} // Funcion que devuelve un puntero a FILE , la funcion no recibe parametros de entrada *(void)*

void vaciar_staging(void) {
    FILE *clear = fopen(".ugit/staging_area.txt", "w");
    if (clear) fclose(clear);
} //Funcion que vacia el contenido de staging_area.txt 
...
void generar_hash(char *dest, size_t len) { // len = longitud maxima de caracteres, dest = buffer donde se guarda el hash
    ...
    for (size_t i = 0; i < len - 1; i++) //ciclo for con i incializado en 0. al ser size_t solo contempla numeros positivos
        dest[i] = chars[rand() % 16]; //arreglo que va conformando el id del commit, con caracteres de 0-f (hexadecimal)
    dest[len - 1] = '\0'; 
}
// ---------------- ADD ----------------
...
void ugit_commit(const char *mensaje) {
    ...
    FILE *file_staging = abrir_staging_para_lectura();
    if (!file_staging) {
        printf("No existe staging area. Ejecuta ugit init.\n");
        return;
    }
    // Verificar si staging está vacío
    fseek(file_staging, 0, SEEK_END);
    if (ftell(file_staging) == 0) {
    ...
    while (fgets(nuevo.archivos[nuevo.total_archivos], sizeof(nuevo.archivos[0]), file_staging)) {
        ...
        if (strncmp(linea, "commit", 6) == 0) {
            ...
            if (sscanf(linea, "commit %8s", id) == 1) {
                if (strcmp(id, commit_id) == 0) { //se ha encontrado el commit
                    ...
                    mostrando = false; //si la id almacenada no coincide con commit_id mostrando seguira en estado FALSE
                }
            }
        }
